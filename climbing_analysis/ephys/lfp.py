...
def epoch_lfp(data, dflist, frame_captures, stances, node='r_forepaw',epoch_loc='start',xlim_=[-0.5,0.5], bin_size=0.02, smooth_sigma=1.0, fs=30000, prune_trials=True,save_fig=None):
    ...
    tstamps_pose = np.linspace(xlim_[0],xlim_[1],200)
    tstamps_ephys = np.linspace(xlim_[0],xlim_[1],60000)
    ...
    for ii in range(len(frame_captures)):
        ...
        for i, tstart in enumerate(event_times):
            ...
            erp_ = data[tstart_ds-int(fs_ds):tstart_ds+int(fs_ds)] #- np.mean(data[tstart_ds-int(fs_ds/2):tstart_ds])
            erps.append(erp_)
            kinematics.append((movement[tstart-100:tstart+100]-movement[tstart])*pixels_to_cm())
...
def morlet_lfp(data,dflist,frame_captures,stances,node='r_hindpaw',epoch_loc='start',freqs=np.arange(2,40,1), n_cycles=None, save_fig=None):
    erps,kinematics,fs = epoch_lfp(data,dflist,frame_captures,stances,node=node,epoch_loc=epoch_loc)
    ...
    mu = power[:,:,int(fs/2):int(fs)].mean(axis=2,keepdims=True)
    power_z = ((power-mu)/mu)*100
    #power_db = 10*np.log10(power_z)
    power_avg = power_z.mean(axis=0)
    times = np.linspace(-1,1,int(fs*2))
    # Plot
